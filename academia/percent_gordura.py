...
def pollock_schimidt_jackson_1980(dobras, pessoa):
    ...
    if pessoa.sexo == 2:
        densidade = Decimal(1.0994921) - Decimal(0.0009929) * soma + \
                    Decimal(0.0000023) * (soma**2) - \
                    Decimal(0.0001392) * pessoa.idade
    else:
        densidade = Decimal(1.10938) - Decimal(0.0008267) * soma + \
                    Decimal(0.0000016) * (soma**2) - \
                    Decimal(0.0002574) * pessoa.idade
    return densidade2gordura(densidade, pessoa)
...
def pestroski_1995_homem(dobras, pessoa):
    soma = dobras.subescapular + dobras.tricipes + dobras.suprailiaca + \
           dobras.panturrilhamedia
    densidade = Decimal(1.10726863) - Decimal(0.00081201) * soma + \
                Decimal(0.00000212) * (soma**2) - \
                Decimal(0.00041761) * pessoa.idade
    return densidade2gordura(densidade, pessoa)
